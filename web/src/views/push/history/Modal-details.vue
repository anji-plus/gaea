      ...
      <el-row type="flex" justify="center" class="pt20">
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
          <div class="grid-content">
            <div class="right-box">
              <div class="log-list" v-if="mailDetails!==undefined">
                <div class="l-mailDemo">
                  <ul v-if="temolateType =='mail'">
                    <li><label>收件人：</label><div>{{mailDetails.pushTo}}</div></li>
                    <li><label>发送时间：</label><div>{{dateFormat(mailDetails.createdTime)}}</div></li>
                    <li><label>内容：</label>
                      <div class="mail-detail" v-html="mailDetails.content"></div>
                    </li>
                    <li><label>发送状态：</label>
                      <div v-if="mailDetails.sendStatus==1" style="color: #55a532">成功</div>
                      <div v-if="mailDetails.sendStatus==0" style="color: red">失败</div>
                    </li>
                    <li><label>返回参数：</label><div>{{mailDetails.sendResult}}</div></li>
                  </ul>
                  <ul v-else>
                    <li><label>收件人：</label><div>{{mailDetails.pushTo}}</div></li>
                    <li><label>发送时间：</label><div>{{dateFormat(mailDetails.createdTime)}}</div></li>
