...
// 数据字典映射对象
export function setDictCodes(list) {
  const obj = {}
  list.forEach((item) => {
    obj[item.bizCode] = item.bizText
  })
  return obj
}
...
export function dialogTypeFunc(dialogType) {
  let typeTitle = ''
  switch (dialogType) {
    case 'ADD':
      typeTitle = i18n.t('btn.add')
      break
    case 'EDIT':
      typeTitle = i18n.t('btn.edit')
      break
    case 'VIEW':
      typeTitle = i18n.t('btn.view')
      break
    default:
      break
  }
