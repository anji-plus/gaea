<template>
  ...
  <transition name="modal-fade">
    <!-- 关闭模态框事件 和 控制模态框是否显示 -->
    <div v-show="show" class="modal-backdrop" @click="$emit('closeModal')">
      <div class="modal" @click.stop>
        <div id="modalDescription" class="modal-body">
          <!-- 状态提示文字的插槽 -->
          <slot name="status">{{ statusText }}</slot>
          <slot name="body" />
          <el-button class="icon-btn_style fr" style="font-size: 24px" @click="$emit('closeModal')"> × </el-button>
        </div>
        <!-- 模态框内容文字 可有可无 -->
        <div v-if="contentText" class="modal-content">
